...
export function StoryForm({ defaultLevel, targets }: {
  defaultLevel: string; targets: TargetOption[];
}) {
  ...
  return (
    <form action={action} className="panel story-form">
      ...
      <div className="field">
        <label htmlFor="topic">Topic <span className="muted">(optional)</span></label>
        <input id="topic" name="topic" placeholder="Moving to a new city…" />
      </div>

      <fieldset className="target-picker">
        <legend>Target words <span className="muted">· auto-select if empty</span></legend>
        <div className="target-option-grid">
          {targets.map((target) => (
            <label className="target-option" key={target.lexemeId}>
              ...
              <span><strong>{target.label}</strong><small>{target.state.toLowerCase()}</small></span>
            </label>
          ))}
        </div>
      </fieldset>
      ...
      {state.status === "success" ? (
        <StatusNotice tone="success">
          {state.message}
          {state.storyId ? <Link href={"/stories/" + state.storyId} className="status-link">Read <ArrowRight size={15} /></Link> : null}
        </StatusNotice>
      ) : null}

      <ActionButton pendingLabel="Writing…"><BookOpenText size={18} />Create story</ActionButton>
    </form>
  );
}
