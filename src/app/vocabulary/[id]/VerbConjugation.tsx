...
export function VerbConjugation({ lexemeId }:{ lexemeId:string }){
  ...
  return <>
    <button className="word-quick-action-button" type="button" onClick={()=>setOpen(true)}>
        ...
        {query.data?<>
          ...
          <div className="conjugation-tabs" role="tablist" aria-label="Conjugation tense">
            {tabs.map((tab)=><button
              ...
              type="button"
              role="tab"
              aria-selected={selected?.id===tab.id&&active!=="imperative"&&active!=="forms"}
              aria-controls="conjugation-tense-panel"
              className={selected?.id===tab.id&&active!=="imperative"&&active!=="forms"?"is-active":""}
              onClick={()=>setActive(tab.id)}
            >{tab.label}<small>{tab.german}</small></button>)}
          ...
          <section id="conjugation-tense-panel" className="conjugation-content" role="tabpanel" aria-live="polite">
            {active==="imperative"?<div className="conjugation-reference-grid">
              ...
              <div className="conjugation-tense-heading">
                <strong>{selected.tense.label}</strong>
                {selected.tense.note?<span>{selected.tense.note}</span>:null}
              </div>
              <table className="conjugation-table">
                <thead><tr><th>Person</th><th>Conjugated form</th></tr></thead>
                <tbody>{selected.tense.forms.map((row)=><tr key={row.person}><th scope="row">{row.person}</th><td>{row.form}</td></tr>)}</tbody>
              </table>
            </>:null}
          </section>
        </>:null}
